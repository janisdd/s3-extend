...
class RpiGateway(MyGatewayBase):
    ...
    def __init__(self, *subscriber_list, **kwargs):
        ...
        self.servo_PCA9685 = PCA9685(0x40, debug=False)
        # use   pwm.setServoPulse(0,X) where X ranges from 500 to 2500 !!
        self.servo_PCA9685.setPWMFreq(50) # DO NOT CHANGE will go out of 180 degree range
    ...
    def get_IR_key(self):
        ...
        while n < 20_000:
            n += 1
            if self.pi.read(IR) == 0:
                count = 0
                while self.pi.read(IR) == 0 and count < 200:  # 9ms
                    count += 1
                    time.sleep(0.00006)
                if (count < 10):
                    return;
                count = 0
                while self.pi.read(IR) == 1 and count < 80:  # 4.5ms
                    count += 1
                    time.sleep(0.00006)
